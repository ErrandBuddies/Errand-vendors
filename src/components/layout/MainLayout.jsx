...
const MainLayout = () => {
  ...
  const getUnreadMessages = () => {
    chatService.getConversations().then(({data}) => {
      const unreadCount = data.filter((conversation) => conversation.unread).length;
  ...
  const navItems = [
    ...
    { name: "Messages", icon: MessageSquare, path: ROUTES.MESSAGES, badge: unreadMessages },
  ];

  return (
    <div className="min-h-screen bg-background flex flex-col">
      {/* Header */}
      {getPageTitle() !== "Messages" && (
          ...
          <div className="px-4 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button className="relative p-2 hover:bg-gray-100 rounded-full">
                ...
                <span className="absolute top-1.5 right-1.5 w-fit p-1 h-fit bg-secondary rounded-full"></span>
              </button>
              <Avatar
            ...
            return (
              ...
              >
                {item.badge && Number(item.badge) > 0 && (
                  <span className="absolute top-1.5 right-1.5 w-fit p-1 h-fit bg-secondary rounded-full">
                    {item.badge}
                  </span>
                )}
